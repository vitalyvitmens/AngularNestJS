...
//TODO: Custom Query Method (Пользовательский метод запроса)
// На этом этапе мы закончим back-end, чтобы у нас был полный API, готовый к использованию нашим front-end. Во-первых, мы добавим новый маршрут под названием «query», который позволит нам получить все маршруты, соответствующие запросу пользователя на front-end (например, «give me all the flights from Jackson to Chicago» (дайте мне все рейсы из Джексона в Чикаго)).
// См. Рис: Custom Query Method.jpg
// Давайте разберем это. Это метод Get с формой URL-адреса query/CITY1/CITY2, где «city1» и «city2» представляют города отправления и назначения соответственно. Тогда это асинхронный запрос, означающий, что JavaScript должен дождаться готовности результатов. Затем мы объявляем два наших входящих параметра города как orig и dest соответственно. Наконец, мы возвращаем запрос из нашей service полетов, которая фактически обращается к TypeORM для выполнения запроса. Теперь давайте посмотрим на соответствующий метод обслуживания рейсов:
// См. Рис: Custom Query Method_2.jpg
// В службе полета мы вызываем встроенный в репозиторий метод " find " ORM, который принимает два искомых параметра в виде пар ключ/значение и выполняет запрос за нас. Результат, когда мы делаем вызов URL-адреса следующим образом: http://localhost:3002/flights/query/Jackson/Chicago
// Мы получаем это:
// См. Рис: Custom Query Method_3.jpg
// Мы рекомендуем вам просмотреть полный код на GitHub, чтобы увидеть, как контроллер полетов и полеты службы работают вместе, чтобы сделать эти красивые запросы. Посмотреть на GitHub:
// https://github.com/SoloLearn-Courses/nest_flights/tree/step2/src/flights
// TypeORM имеет встроенный метод «запроса», который может принимать необработанный SQL, что позволяет вам создавать собственные пользовательские запросы.

//TODO: Adding a Flight (Добавление рейса)
// Если мы хотим добавить рейс в систему, в RESTful API мы делаем POST -запрос к коллекции. В этом случае мы бы сделали POST -запрос к flights route (маршруту перелета). Вот как это выглядит:
// См. Рис: Adding a Flight.jpg
// Давайте разберем это. Глагол http используется здесь вместо GET POST. Этот метод представляет собой асинхронный метод под названием «create», он использует декоратор @Body и принимает один параметр, представляющий собой пакет информации, который мы называем «flight». Тело (@Body) POST-запроса — это пакет входящих данных. У каждого back-end есть свой способ доступа к body post request, но по сути это пакет данных, поступающих от front-end в форме объекта JavaScript или аналогичной структуры данных, в зависимости от того, на каком языке написан ваш back-end. Теперь давайте посмотрим на соответствующий flight service method (метод обслуживания рейсов):
// См. Рис: Adding a Flight.jpg_2.jpg
// эти два метода вместе помогают нам создать новый рейс, поступающий из front-end. Посмотреть на GitHub:
// https://github.com/SoloLearn-Courses/nest_flights/blob/step3/src/flights/flights.controller.ts
// BODY post request содержит всю информацию, необходимую back-end для вставки нового элемента в базу данных.

//TODO: Updating a Flight (Обновление рейса)
// Если мы хотим обновить полет в системе, в RESTful API мы делаем запрос PATCH. Вот как это выглядит:
// См. Рис: Updating a Flight.jpg
// Давайте разберем это. Глагол http, используемый здесь вместо POST, — это PATCH. Думайте о «patch» (заплатке), как о заделывании дыры в свитере. Вы не заменяете весь свитер, вы просто латаете одну его часть. Допустим, вы хотите обновить время полета с 9:00 до 9:45 из-за задержки. Ты бы починил. Патч похож на POST в том смысле, что он отправляет пакет данных на сервер, но также отправляет идентификатор в URL-адресе. Таким образом, вы можете отправить запрос на исправление по адресу: http://localhost:3002/flights/8/update.
// Это поставит заплатку полету номер 8 с пакетом данных, которые вы отправляете, который содержит ключи полей, которые вы хотите исправить. Теперь давайте посмотрим на соответствующий flight service method (метод обслуживания рейсов):
// См. Рис: Updating a Flight_2.jpg
// эти два метода вместе помогают нам update (обновить) или «patch» (исправить) существующий рейс в системе. Посмотреть на GitHub:
// https://github.com/SoloLearn-Courses/nest_flights/blob/step3/src/flights/flights.controller.ts
// Edit, update, and PATCH («Редактировать», «обновить» и «ИСПРАВИТЬ») — все это слова, описывающие почти одно и то же. PATCH — это HTTP-глагол.

//TODO: Deleting a Flight (Удаление рейса)
// Если мы хотим удалить рейс в системе, в RESTful API делаем запрос DELETE. Вот как это выглядит:
// См. Рис: Deleting a Flight.jpg
// Давайте разберем это. Здесь используется HTTP-глагол DELETE. Метод удаления требует только одного: идентификатора. Пакет информации не нужен. Почему? Что ж, для того, чтобы удалить ресурс, нам нужно знать только его идентификатор в базе данных. Вот и все… других вопросов нет.
// Теперь давайте посмотрим на соответствующий flight service method (метод обслуживания рейсов):
// См. Рис: Deleting a Flight_2.jpg
// эти два метода вместе помогают нам удалить существующий рейс в системе. Посмотреть на GitHub:
// https://github.com/SoloLearn-Courses/nest_flights/blob/step3/src/flights/flights.controller.ts
// Для удаления записи в большинстве RESTful API требуется только идентификатор.

//TODO: CORS (КОРС)
// Прежде чем мы пойдем дальше, мы должны обсудить важную концепцию веб-безопасности: совместное использование ресурсов между источниками или CORS. При локальном запуске нашего full stack locally, NestJS по умолчанию работает на порту 3000 (мы запускаем наш на порту 3002), а Angular по умолчанию работает на порту 4200. Когда я пытаюсь сделать API-вызовы из моей службы Angular в мой NestJS back-end, я получаю сообщение об ошибке:
// См. Рис: CORS.jpg
// это связано с тем, что существуют правила по умолчанию, чтобы убедиться, что компьютер не пытается получить доступ к серверу в другом домене без разрешения (мы запускаем эти два ресурса на разных портах, поэтому они являются отдельными). Обработка CORS — это огромная тема, но для наших целей локального тестирования есть быстрое решение. Мы можем использовать Chrome для всех наших тестов, и в Chrome есть плагин под названием Allow CORS: Access-Control-Allow-Origin. Когда этот плагин установлен и активирован в вашем браузере, Chrome позволит двум ресурсам (NestJS и ANGULAR) через их разные порты взаимодействовать друг с другом. Этот конкретный плагин изменил внешний вид с годами, но по состоянию на 2020 год вот как он выглядит, когда он включен или отключен:
// См. Рис: CORS_2.jpg
// ПРЕДУПРЕЖДЕНИЕ! Если вы используете плагин Chrome Allow CORS: Access-Control-Allow-Origin, обязательно отключите его после завершения. Если этот плагин включен, он может мешать аутентификации/авторизации на таких сайтах, как Gmail, Facebook, YouTube и некоторых других онлайн-ресурсах. Выключите его, когда закончите, иначе будет казаться, что некоторые части Интернета не работают.
// Совместное использование ресурсов из разных источников или CORS по умолчанию не разрешено в большинстве браузеров, но существуют подключаемые модули браузера, которые можно использовать для решения этой проблемы в целях тестирования на вашем компьютере.

//TODO: Data on Demand (Данные по запросу)
// Теперь, когда наши серверы работают и у нас есть полностью функциональная back-end, все, что нам нужно сделать, — это обеспечить полную связь front-end с нашим back-end. Самое первое, что мы сделаем, это сделаем так, чтобы рейсы больше не загружались в ngOnInit в нашем файле home.component.ts. Мы хотим загружать рейсы только тогда, когда нажимаем кнопку «Перейти». Итак, мы создадим новый метод в нашем файле home.component.ts с именем query() и подключим кнопку «go» к методу запроса в нашем компоненте.
// home.component.html:
// См. Рис: Data on Demand.jpg
// home.component.ts:
// См. Рис: Data on Demand_2.jpg
// Теперь полеты не будут загружаться, если мы не нажмем кнопку, которая СООБЩАЕТ их загрузить.
// В Angular компоненты и views (представления) тесно взаимодействуют друг с другом для управления взаимодействием с пользователем.

//TODO: Flight Queries (Запросы на рейс)
// Теперь давайте настроим наш запрос рейсов таким образом, чтобы вместо получения ВСЕХ рейсов отображались только те рейсы, которые мы запросили. Мы добавим ngModel в меню выбора источника и пункта назначения соответственно. Каждый из них будет выглядеть примерно так:
// См. Рис: Flight Queries.jpg
// Затем в нашем компоненте мы отправим данные ngModel из нашего меню выбора прямо на маршрут запроса back-end NestJS (наш маршрут выглядит следующим образом): /flights/query/city1/city2 Мы объявит наши модели selectedOrigin и selectedDestination в файле home.component.ts, а затем использует эти модели для хранения данных двух городов, выбранных пользователем. В нашем файле home.component.ts мы отправим в сервис два города:
// См. Рис: Flight Queries_2.jpg
// Затем наш сервис необходимо обновить, чтобы сделать вызов API к методу запроса в нашем бэкэнде:
// См. Рис: Flight Queries_3.jpg
// теперь у нас есть полный запрос, который вытягивает полеты из нашего бэкенда в наш интерфейс на основе выбора пользователя. У нас есть приложение с полным стеком, работающее от начала до конца, и данные, которые проходят весь путь от базы данных через сервер NestJS до нашего внешнего интерфейса на основе нашего пользовательского запроса.
// Попробуйте на StackBlitz:
// https://stackblitz.com/github/SoloLearn-Courses/TSFlights1/tree/step8?file
// Посмотреть на GitHub:
// https://github.com/SoloLearn-Courses/TSFlights1/blob/step8/src/app/flights.service.ts
// Помните, что мы можем использовать ngModels для хранения данных в представлении, которое мы можем позже получить в компоненте.

//TODO: Admin Panel (Панель администратора)
// Мы прошли долгий путь в нашем путешествии по Angular и NestJS. Наша последняя задача теперь будет заключаться в отправке данных на наш back-end. После того, как мы отправили данные и получили данные из нашем back-end, у нас есть full cycle, full stack application (приложение полного цикла с полным стеком). Самое первое, что нам нужно сделать, это создать форму на стороне Angular, чтобы отправить новый рейс обратно на сервер. Это достигается с помощью простой html-формы во front-end, которая использует [(ngModel)] для отслеживания каждого значения в форме:
// Исследуйте StackBlitz.
// https://stackblitz.com/github/SoloLearn-Courses/TSFlights1/tree/step9?file=README.md
// Теперь мы отправим запрос POST, чтобы отправить новый рейс на наш сервер. Ключевым моментом для того, чтобы это заработало, является сохранение всех наших переменных полета в ngModels, а затем отправка их в наш сервис Angular. Вот что у нас Файл admin.component.ts выглядит так:
// См. Рис: Admin Panel.jpg
// Здесь мы, по сути, собираем все данные, которые нам нужны для создания объекта полета, а затем отправляем их в наш service, который выглядит так:
// См. Рис: Admin Panel_2.jpg
// -Конец, и вуаля! Теперь у нас есть полная возможность создавать и искать рейсы в нашем приложении. Запросы POST, в отличие от запросов GET, отправляют данные о наших полетах за кулисами, не будучи видимыми в браузере. Это очень важно, потому что мы не хотим раскрывать данные, поступающие в нашу базу данных, случайным observers (наблюдателям) через Интернет. Вы можете просмотреть разделы администратора в примерах StackBlitz и/или GitHub ниже: Изучить на StackBlitz:
// https://stackblitz.com/github/SoloLearn-Courses/TSFlights1/tree/step9?file=README.md
// Посмотреть на GitHub:
// https://github.com/SoloLearn-Courses/TSFlights1/blob/step9/src/app/admin/admin.component.ts
// ПОЗДРАВЛЯЕМ! Теперь вы познакомились с несколькими инструментами, которые можно использовать для создания полнофункционального приложения TypeScript с нуля.

//TODO: Dynamically Loading Cities (Динамическая загрузка городов)
// До этого момента мы жестко закодировали выбор города в раскрывающемся списке запросов на главной странице:
// См. Рис: Dynamically Loading Cities.png
// Вот соответствующий HTML-код в нашем представлении для этого раскрывающегося меню:
// См. Рис: Dynamically Loading Cities_2.png
// Проблема, которую нужно решить здесь, заключается в том, что у нас есть раскрывающийся список городов, которые должны быть получены из базы данных, чтобы они не были жестко закодированы в меню выбора. Таким образом, при добавлении, обновлении или удалении названий городов раскрывающееся меню будет точно отображать обновленный список городов каждый раз при загрузке этой страницы. Наша стратегия будет заключаться в том, что мы напишем новые запросы, чтобы получить только названия городов отправления и назначения рейсов. Затем мы пройдемся по этим городам и заполним наши выпадающие меню во front end этими динамическими названиями городов из базы данных.
// *ngFor может перебирать буквально любой элемент html, а не только теги div. В этом случае мы будем использовать *ngFor для перебора параметров в нашем меню выбора для динамического заполнения городов.

//TODO: Get Dynamic Cities from Database (Получить динамические города из базы данных)
// Теперь мы реализуем стратегию, изложенную в предыдущей части.
// Во-первых, мы добавляем два метода в наш файл Flight.service.ts, которые получают города отправления и назначения рейсов соответственно:
// Flights.service.ts в Angular:
// См. Рис: Get Dynamic Cities from Database.jpg
// Теперь в нашем back end коде NEST нам нужно добавить метод контроллера, соответствующий тому, что вызывает наш front end:  flights/cities/origins and flights/cities/destinations respectively (соответственно):
// Flights.controller.ts в NEST:
// См. Рис: Get Dynamic Cities from Database_2.jpg
// Затем мы создаем новые методы в FlightService на нашем сервере Nest для обработки этих новых запросов:
// Flights.service.ts в NEST:
// См. Рис: Get Dynamic Cities from Database_3.jpg
// Здесь мы используем метод запроса TypeORM для написания пользовательского запроса, который получает DISTINCT города из нашей таблицы рейсов, что удалит все дубликаты при получении пунктов отправления и назначения соответственно.
// Теперь, когда мы попадаем на наш маршрут http://localhost:3002/flights/cities/origins, мы получаем массив объектов с ключом «origin» и значением, являющимся каждым из уникальных городов в нашей базе данных.
// Пример наших данных теперь из вызова API Flights/Cities/Origin:
// [{"origin":"Phoenix"},{"origin":"London"}{"origin":"Lima"},{"origin":"Bogota"}] В нашей следующей части урока мы закончим front end вид, чтобы ввести этот новый список динамических, уникальных городов из back end.
// Посмотреть на StackBlitz:
// https://stackblitz.com/github/SoloLearn-Courses/TSFlights1/tree/step9a
// Посмотреть на GitHub:
// https://github.com/SoloLearn-Courses/TSFlights1/tree/step9a
// Посмотреть серверную часть на GitHub:
// https://github.com/SoloLearn-Courses/nest_flights/tree/step4
// Ключевое слово SQL DISTINCT можно использовать для возврата всех уникальных экземпляров значения в столбце базы данных при удалении дубликатов.

//TODO: Pull In Dynamic Cities (вытащить динамические города)
// Последнее, что нам нужно сделать, это вытащить динамические города из back end в наш front end Angular. Мы уже вызываем базу данных в Flight.service.ts в Angular, которая вызывается home.component.ts, но наше представление еще не показывает новые данные о городе. Давайте построим это сейчас. В нашем файле home.component.ts мы создали списки с именами filteredOriginList и filteredDestinationList, каждый из которых выглядит примерно так (с собственными уникальными городами, извлеченными из back end базы данных):
// [{"origin":"Phoenix"},{"origin":"London"},{"origin":"Lima"},{"origin":"Bogota"}]
// С нашей точки зрения, мы можем изменить нашу опцию меню выбора, чтобы она выглядела следующим образом:
// Typescript (TS):
{/* <option *ngFor ="let result of filteredOriginList; let i=index" 
value={{result.origin}} >{{result.origin}}</option>  */}
// По сути, список городов представляет собой массив объектов, где ключом является origin (происхождение) слова, а значением является unique (уникальный) город в каждой записи из базы данных. Это заполнит раскрывающееся меню на нашей домашней странице всеми уникальными городами в базе данных, поэтому нам не нужно их жестко кодировать. Мы повторяем этот процесс с городами назначения, и теперь, наконец, наши города отправления и назначения динамически заполняются на нашей странице рейсов, где мы ищем рейсы! Полный код легче увидеть в нашем живом примере, поэтому не стесняйтесь исследовать код на StackBlitz или GitHub: Посмотреть на StackBlitz:
// https://stackblitz.com/github/SoloLearn-Courses/TSFlights1/tree/step9b
// Посмотреть на GitHub:
// https://github.com/SoloLearn-Courses/TSFlights1/tree/step9b
// Всегда знайте свои данные. Иногда полезно сделать вызов API в браузере, просто чтобы посмотреть, как возвращаются данные, чтобы вы могли правильно выполнить итерацию их на внешнем интерфейсе.

//TODO: Admin Flight Table (Таблица полетов администратора)
// Чтобы обеспечить возможность обновления рейсов, нам нужно увидеть список всех рейсов, чтобы решить, какие рейсы удалить. В файле admin.component.ts мы получим все полеты в методе ngOnInit так же, как и в домашнем компоненте. На этот раз нам не нужно просто брать источник; мы получим все полные полеты со всеми их данными. В admin.component.ts мы можем просто сделать:
// TypeScript (TS):
// ngOnInit(): void {
...
//   })
// }
// Затем, на наш взгляд, мы можем перебрать все полеты в таблице в view (представлении):
...
// Обратите внимание на часть let i=index цикла *ngFor. Это важно; мы не используем индексы на этом шаге кода, но воспользуемся ими позже. Эти индексы похожи на индексы в цикле for; они начинаются с нуля и считаются вверх. Для экономии места мы максимально сократили текст, чтобы все эти столбцы поместились на экране мобильного устройства. Вот как выглядит наше представление сейчас: 
// См. Рис: Admin Flight Table.png
// Из соображений безопасности нам пришлось отключить обновление сборок на рабочем сервере, но вы можете просмотреть код в реальном времени на StackBlitz или загрузить front and back-end вместе на свой компьютер, чтобы протестировать все локально: 
// Просмотр на StackBlitz:
// https://stackblitz.com/github/SoloLearn-Courses/TSFlights1/tree/step9b?file=README.md
// Посмотреть на GitHub:
// https://github.com/SoloLearn-Courses/TSFlights1/tree/step9b/src/app/admin
// Посмотреть back-end на GitHub:
// https://github.com/SoloLearn-Courses/nest_flights/tree/step3
// При переборе массива в представлении Angular, если вы используете таблицу, обычно лучше всего реализовать *ngFor для каждой строки таблицы (tr) по мере прохождения данных. Таким образом, каждая строка таблицы представляет одну строку данных из вашей базы данных.

//TODO: Update Flight Strategy (Обновить стратегию полета)
// Держитесь за шляпы, скоро будет весело! Теперь, когда у нас есть список рейсов, пришло время обновить рейс. В Angular так много инструментов, что фактический процесс обновления полета может выполняться (и выполняется) несколькими различными способами. Способ, который мы будем демонстрировать здесь, использует очень простые инструменты, которые вы уже изучили. Давайте поговорим о нашем процессе. Что мы сделаем: мы сделаем каждый элемент в списке полетов, мы получаем ngModel, чтобы он был динамическим. Затем мы вместо того, чтобы просто перечислять рейсы, сделаем каждое поле в рейсах редактируемым, сделав его текстовым вводом. Затем, когда мы обновим рейс, мы передадим весь объект рейса в метод обновления. Метод обновления будет знать, какой рейс нужно обновить в базе данных, потому что «id» является частью данных, которые мы получаем из нашего вызова API. Здесь важно отметить, что у нас есть ДВА идентификатора.... есть идентификатор, который относится к последовательному идентификатору каждого рейса в нашем массиве рейсов, а затем есть идентификатор базы данных, который является одним из полей каждого рейса. Убедитесь, что вы держите их прямо. Это распространенный сценарий в front-end веб-разработке: идентификаторы на основе массива используются для перебора вещей в представлении и отслеживания того, с какими элементами мы взаимодействуем, а идентификаторы базы данных используются позже, когда мы отправляем элемент в базу данных, получить обновление или удалить. 
// ИЗМЕНЕНИЯ В ПОЛЯХ ДАННЫХ: 
// В нашем front-end мы меняем каждое поле полетных данных примерно так:
{/* <td>{{flight.origin}}</td> */}
// Что-то вроде этого:
{/* <td><input [(ngModel)]='flight.origin'></td>  */}
// ИЗМЕНЕНИЯ КНОПКИ ОБНОВЛЕНИЯ: 
// Затем мы меняем нашу кнопку обновления :
{/* <td><button class="update">go</button></td>  */}
// К этому:
{/* <td><button class="update" (click)="update(flight)">go</button></td>  */}
// Примечание: из-за нехватки места в представлении для мобильных устройств мы не сделаем все поля редактируемыми. Мы создадим три редактируемых поля: пункт отправления, пункт назначения и номер рейса. Теперь мы можем «редактировать на месте» эти первые три поля, просто введя новые значения, которые мы хотим, и нажмите «go» в столбце обновления. Наш метод обновления получает новые значения, которые нам нужно отправить в базу данных в нашей следующей части урока. 
// Посмотреть на StackBlitz:
// https://stackblitz.com/github/SoloLearn-Courses/TSFlights1/tree/step9c?file
// Посмотреть на GitHub:
// https://github.com/SoloLearn-Courses/TSFlights1/blob/step9c/src/app/admin/admin.component.html
// Посмотреть серверную часть на GitHub:
// https://github.com/SoloLearn-Courses/nest_flights/tree/step3
// Любая переменная с уникальным именем в компоненте может быть преобразована в ngModel. Хотя в Angular есть много способов отправить данные формы из представления в компонент, ngModels — это простой способ выполнить эту задачу.

//TODO: Finish Updating Flights (Завершить обновление рейсов)
// Забегая вперед, если мы обновили все полеты в системе, мы хотим, чтобы страница каким-то образом обновлялась, чтобы показать изменения. Пользователям обычно нравится визуальное изменение, которое сигнализирует о том, что изменение произошло. На самом деле, когда мы удаляем рейс (который появится в следующей части урока), мы также хотим, чтобы все рейсы обновлялись в системе. Давайте спланируем это сейчас. Изначально мы загружали все полеты в нашу админку в методе ngOnInit. Давайте вместо этого создадим главный метод обновления, который можно вызывать всякий раз, когда он нам понадобится. Это будет при загрузке страницы, обновлении рейса и удалении рейса. Мы можем переместить код загрузки рейса из метода ngOnInit в метод обновления как таковой. В admin.component.ts мы можем сделать это:
// refresh(){
//   this.flightService.getAllFlights().subscribe(data =>{
//     this.flightList = data;
//   })
// } 
// Тот же функционал, другое место. Теперь мы можем вернуться к нашему методу ngOnInit и вызвать наш новый многоцелевой метод обновления:
// ngOnInit(): void {
//   this.refresh();
// } 
// Великолепный. Теперь все, что нам нужно сделать, это создать метод обновления, который отправляет рейс в нашу службу полетов:
...
//   });
//  }
 // Утверждение «if» важно. Если мы получили данные, и данные представляют собой объект с «affected» (затронутым) ненулевым значением, то это довольно хороший признак того, что обновление сработало на основе того, как построена наша форма. Мы знаем, что у него будет «affected» ключ в ответе API из изучения ответа API, возвращаемого NestJS. При этом все, что нам нужно сделать, это разработать метод обновления в нашем файле FlightService.ts:
//  updateFlight(flight: Flight) {
//   return this.http.post(`${this.backEndURL}/flights/${flight.id}/update`,flight);
// }
// Это завершает нашу функциональность обновления. Вы можете просмотреть этот новый код на StackBlitz, но помните, что по соображениям безопасности мы не можем позволить пользователям обновлять живой код в Интернете, поэтому эта функциональность отсутствует. Тем не менее, вы можете просмотреть как front-end код Angular, так и back-end код NestJS, скачать их и запустить вместе, если хотите увидеть, как они работают вживую: 
// Посмотреть на StackBlitz:
// https://stackblitz.com/github/SoloLearn-Courses/TSFlights1/tree/step9d
// Посмотреть front-end код Angular на GitHub:
// https://github.com/SoloLearn-Courses/TSFlights1/tree/step9d
// Посмотреть back-end код Nest на Github:
// https://github.com/SoloLearn-Courses/nest_flights/tree/step3
// Помните, что наш метод обновления рейса передает идентификатор рейса, который нужно обновить, в URL-адресе вместе с набором данных, представляющих, как должен выглядеть новый рейс. По сути, мы нажимаем на такой URL-адрес: https://backend/flights/4/update и отправляем ему объект полета в почтовом запросе как часть тела запроса.

//TODO: Delete Flights Method (Метод удаления рейсов)
// Наконец, нам нужно закодировать процесс удаления рейса. Возможно, вы помните, что удаление — это просто. Все, что нам нужно сделать, это передать идентификатор back-end route удаления. Это будет очень похоже на наш метод обновления, но нам не нужно передавать какие-либо пакеты данных. Вот наша кнопка обновления с обновленной кнопкой «delete» под ней в admin.component.html:
// HTML:
{/* <td><button class="update" (click)="update(flight)">go</button></td>
<td><button class="delete" (click)="delete(flight)">go</button></td> */}
// И наш метод удаления в admin.component.ts:
...
//   });
// } 
// И, наконец, наш метод удаления в файле Flight.server.ts, который отправляет это на сервер:
// deleteFlight(id: number) {
//   return this.http.post(`${this.backEndURL}/flights/${id}/delete`, null);
//  } 
// Все почтовые запросы требуют как минимум два параметра: первый — это URL-адрес, по которому вы публикуете, а наш второй параметр — это связанный пакет данных. Поскольку данных для отправки нет, мы должны указать null в качестве заполнителя, чтобы удовлетворить минимум 2 параметра для post запросов. Обратите внимание, что мы используем POST-запросы как для обновления, так и для удаления, хотя эти два метода связаны с HTTP-глаголами patch и delete соответственно. Причина этого в том, что на момент написания этой статьи некоторые браузеры понимают только запросы POST и GET, исходящие от браузеров. Чтобы это работало во всех браузерах, мы используем запросы POST и GET только для связи с нашим back-end.
// Необязательно: Если вы хотите, вы можете обернуть весь код в файле admin.component.ts внутри условного оператора диалога подтверждения, например:
// if (window.confirm('are you sure you want to delete this flight? ')){
//   // your delete method code 
// } 
// Мы включили эту необязательную строку в репозиторий ниже. 
// Посмотреть на StackBlitz (помните, что операции CRUD на рабочем сервере отключены):
// https://stackblitz.com/github/SoloLearn-Courses/TSFlights1/tree/step9e
// Посмотреть на GitHub:
// https://github.com/SoloLearn-Courses/TSFlights1/tree/step9e
// Посмотреть back-end код NestJS на GitHub:
// https://github.com/SoloLearn-Courses/nest_flights/tree/step3
// Поскольку методы удаления являются быстрыми и простыми, иногда полезно обернуть деструктивные действия, такие как удаление, внутри if оператора подтверждения, просто из вежливости по отношению к вашим пользователям, чтобы люди случайно не заметили удаление важного ресурса.

//TODO: Summary (Резюме)
// Теперь у нас есть full stack application (приложение с полным стеком), написанное полностью на TypeScript со всеми работающими операциями CRUD.
// В двух словах, у нас есть страница рейсов, которая динамически загружает все рейсы из нашего back-end, отображает карты и фильтрует список возможных пунктов отправления и назначения, а также помещает эти параметры в меню выбора для поиска рейсов. Затем у нас есть меню администратора, которое позволяет вам создавать, читать и обновлять все полеты. Мы используем ngModel для отслеживания каждой строки полетов, поэтому вы можете «редактировать на месте» любой данный полет и нажать «Перейти» для обновления. У нас есть метод удаления, который использует всплывающее окно подтверждения для подтверждения удаления рейса. Наконец, мы используем медиа-запросы для изменения размера страницы, чтобы она вел себя по-разному в зависимости от размера экрана. (В StackBlitz на мобильном телефоне загрузка всех данных может занять некоторое время, так что наберитесь терпения). Вот как должна выглядеть панель администратора на мобильном устройстве:
// См. Рис: Summary.png
// Не стесняйтесь загружать переднюю и внутреннюю части соответственно (убедитесь, что вы находитесь в правильных ветках) и экспериментируйте с кодом, расширяйте его и ищите способы улучшить то, что у нас есть. Здесь есть масса соображений на будущее. Мы могли бы добавить какую-то аутентификацию, чтобы пользователи могли войти в систему, прежде чем они начнут возиться с серверной частью. Один из способов сделать это — использовать веб-токены JSON или «JWT» (произносится как «jawts»). JWT создают безопасное соединение между front-end и back-end, чтобы убедиться, что только люди с действительным токеном могут получить доступ к областям с ограниченным доступом. Другим быстрым расширением может быть добавление простого уведомления о загрузке, которое говорит пользователю подождать, пока загружаются полеты из серверной части. Мы пошли дальше и сделали это; просмотрите живой сайт StackBlitz, чтобы увидеть загрузчик в действии.
// ВАЖНО. Если вы запускаете приложение локально, помните, что вам нужен локальный подключаемый модуль CORS для Chrome, чтобы внешний и внутренний домены могли взаимодействовать друг с другом. Выполните поиск в Интернете по запросу «control allow access origin chrome», (управление доступом к исходному хрому) чтобы найти его. ТАКЖЕ: убедитесь, что вы отключили его, прежде чем возобновить обычный просмотр веб-страниц, иначе ключевые веб-сайты в Интернете больше не будут работать. Спасибо за участие в этом курсе! Удачного кодирования. 
// Посмотреть на StackBlitz (помните, что операции CRUD на рабочем сервере отключены):
// https://stackblitz.com/github/SoloLearn-Courses/TSFlights1/tree/step9e?file=README.md
// Посмотреть на GitHub:
// https://github.com/SoloLearn-Courses/TSFlights1/tree/step9e
// Посмотреть внутренний код NestJS на GitHub:
// https://github.com/SoloLearn-Courses/nest_flights/tree/step3
// Наше несвязанное приложение, когда мы запускаем его локально, требует межсайтового скриптинга, потому что у нас есть два разных порта (порт 3002 и порт 4200 соответственно), взаимодействующих друг с другом, которые браузер считает отдельными доменами. Плагин CORS позволяет нам сделать это, ослабив ограничение безопасности.
